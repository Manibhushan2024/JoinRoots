...
<div class="card admin-card shadow-sm border-0">
    <div class="card-header bg-white py-3 border-bottom border-light">
        <h5 class="m-0 fw-bold text-dark"><i class="fas fa-edit me-2 text-primary"></i>Edit Post: {{ $blog->title }}</h5>
    </div>
    <div class="card-body p-4 border border-top-0 rounded-bottom">
        ...
        <form action="{{ route('admin.blog.update', $blog) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-12 mt-4">
                    <div class="form-check form-switch fs-5 p-3 bg-light rounded border border-success border-opacity-25">
                        <input class="form-check-input ms-0 me-3 mt-1" type="checkbox" role="switch" name="is_published" id="isPublished" {{ old('is_published', $blog->is_published) ? 'checked' : '' }}>
            ...
            <div class="mt-4 pt-4 border-top d-flex justify-content-end">
                ...
                <button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i> Update Post</button>
            </div>
        </form>
    </div>
</div>
